...
- Volta manages node, package managers, and Node CLIs. The node which Node CLIs depend on is the default Node version set in Volta.
...
graph TD
  ...
  C --> K(Volta)
    K --> L(pnpm)
    K --> M(Node 20)
    K --> N("Node 22 (LTS)") --> O(Node CLIs installed with Volta)
